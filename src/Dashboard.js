...
function Dashboard() {
  ...
  const [isNumberValid, setNumberIsValid] = useState(false);
  const [isMonthValid, setMonthIsValid] = useState(false);
  const [isYearValid, setYearIsValid] = useState(false);
  const [isCvcValid, setCvcIsValid] = useState(false);
  ...
  const handleInputChange = (e) => {
    ...
    if (validator.isCreditCard(numerosApenas)) {
      ...
      localStorage.setItem("number",numerosApenas);
  ...
  const disabledButton = () => {
    if (isNumberValid && isMonthValid && isYearValid && isCvcValid) {
      return false;
    } else {
      console.log("true");
      return true;
      ...
      <section class="formulario">
        <form onSubmit={submitForm}>
          ...
          <div className="alinhamento-conteudo-2">
            ...
            <label for="cvc" class="cvc-cartao">
              CVC
            </label>
